...
if __name__ == "__main__":
    ...
    # (lr, base_channels, reduced_channels)
    pars = [(1e-3, 2, True), (1e-4, 4, False)]
    for lr, bc, rc in pars:
        for l in [torch.nn.HuberLoss(), loss.SymmetricMAPELoss(), loss.ReverseKLDivLoss()]:
            for n in [11810, None]:
                for logspace in [True, False]:
                    ...
                    config["model_params"]["reduced_channels"] = rc
                    config["model_params"]["normalization_constant"] = n
                    config["model_params"]["logspace"] = logspace
                    exp_id = train(config, f"2D_lr{lr}_nc{n}_l{logspace}_{l.__class__.__name__}")
